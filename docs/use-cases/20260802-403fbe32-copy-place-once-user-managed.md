...
---
# UC-403fbe32: リポジトリの内容を copy で初回だけ配置し、その後はユーザーが手で編集して育てる
...
- 配置物を明示的に撤去する手段があること。copy は世代の外に出るため、世代を戻しても消えず
  撤去には専用の手段が要る。その手段（`reset`）は copy 専用ではなく symlink を含む配置物を
  home / project の両モードで撤去するもので、copy はそれを必要とする唯一の使われ方である
