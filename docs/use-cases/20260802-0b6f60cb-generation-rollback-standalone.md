...
> **上表は `docs/concept.md` からの写し**。use_case は規範を持たないため、配置種別ごとの
...
- 世代を戻した先の記録に無い配置物が保守的に除去され、前世代の配置状態へ収束すること
  （stale 除去そのものは全モードの apply が持つ冪等性の不変条件で、この use_case に固有
  なのは「戻した世代の記録が除去の基準になる」点）
