...
- **撤去系の写像と可逆性** — 撤去の対象が item の全数になること、撤去だけを行う実行では
  symlink の撤去が記録済みの復元先を伴う可逆な変更になり、copy の削除が復元先を持たない
  不可逆な変更になること（削除が捨てるのは記録済みの配置元ではなくディスク上の内容である
  ため）、保守的に残した対象は成功のまま変更を作らないこと（方針としての不作為は差分では
  ない）、撤去系は世代を動かさないためスロット自体が不在であること
