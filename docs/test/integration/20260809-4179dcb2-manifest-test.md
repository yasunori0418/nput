...
type: test_case
name: "manifest_test.go — v1 の復元と不正な版・フィールド・root の拒否"
target: "internal/manifest/manifest_test.go"
covers:
  - "TC-172548ea-154a-4e22-a169-8252a43e3781"
---
# CASE-4179dcb2: manifest_test.go — v1 の復元と不正な版・フィールド・root の拒否
...
- 消費側が知らない版を拒否し、しかも判別可能な値を包んで返すこと。呼び出し側が「pin した
  ...
  判別できることまで確かめる
- 版が下限を下回る（0 および負値）文書を拒否し、しかも版ずれの値では包まないこと。壊れた文書に
  対して呼び出し側が版ずれの案内を出さないことが契約であり、テストは包まれていないことまで
  確かめる
- 未知のキーを黙って無視せず拒否すること
- 評価時に絶対パスが決まる root の種別がパスを持ち、実行時に解決する種別がパスを持たないこと
- root の種別が空である文書と、root 自体が欠けている文書を拒否すること
- 文書が存在しないときに失敗すること
