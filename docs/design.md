...
| **home-manager** | `home.activation` から起動 | `$HOME`（`homeRoot` を pin）| あり（内部・MVP は profile `<name>` = `default` 固定の 1 profile。**役割分離は不可**＝複数 profile は standalone CLI 経路のみ・将来 seam・→ ADR-0024, ADR-0025）| host（`home-manager --rollback`）|
