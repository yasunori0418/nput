...
--quiet             # 廃止予定（現在 no-op）。既定で沈黙するため抑制対象がない（→ ADR-0031）
-v, --verbose       # 配置レポート（サマリ + per-target 行）・内部 nix コマンド詳細を出力（既定は成功時沈黙・→ ADR-0031）
...
- **`-v` / `--verbose` を付けたときだけ配置レポートを stderr に出す**（既定沈黙の opt-in・skip 通知・`apply --all` 完了サマリも含む・→ ADR-0031）。加えて内部実行する nix コマンド等の詳細も出す。`--json` は MVP では持たない（将来送り）。`--quiet` は既定沈黙化に伴い廃止予定（現在 no-op）。
