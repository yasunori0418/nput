...
`lib`（`mkManifest` / マーカー群 / `listFilesInRepo`）は nixpkgs.lib のみに依存する純データ生成器。配置ロジックは持たない（→ ADR-0006）。
...
`manifest.json` は各 entry の解決済み配置元（store パス / out-of-store 絶対パス）・`source`・`target`・`mode`・root の kind（project / home / system / 固定パス）を記録する。
...
| コンポーネント | 依存 |
|---|---|
| `lib/`（`mkManifest` / マーカー群 / `listFilesInRepo`）| nixpkgs.lib のみ（純データ生成。`rsync` 不要）|
| `lib/out-of-store.nix`（`mkOutOfStoreSymlink` / `projectRoot` / `homeRoot` / `systemRoot` マーカー）| なし（純粋なマーカー構築子）|
