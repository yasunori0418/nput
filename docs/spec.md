...
`manifest.json` は各 entry の解決済み配置元（store パス / out-of-store 絶対パス）・`subpath`・`target`・`mode`・root の kind（project / home / system / 固定パス）を記録する（全フィールドは後述「manifest.json スキーマ（v1）」・→ ADR-0013）。
...
- **`normalizeManifest { entries, root } -> attrset`**: `evalModules` 検査・デフォルト適用（`subpath` → `"."` / `mode` → `"symlink"` / `target` → 属性キー）+ クロスフィールド `lib.throwIf` チェック（`mode = "copy"` かつ out-of-store marker / `root = systemRoot` の未実装拒否・→ ADR-0013）+ 内部 marker タグ（`_nputMarker`）→ clean enum（`srcKind` / `rootKind`）変換を行う純データ関数。nix-unit / namaka の単体対象。**識別子の一意性は Nix attrset のキー重複不可で native に担保**し、重複 name の throwIf は持たない（→ ADR-0014）。
...
- `apply --all` は **entrypoint の `nput.*` 定義順に適用**し、**一部が失敗しても残りを続行**する（各 config は独立 profile で atomic なため）。最後に成功 / 失敗を集約表示し、**1 つでも失敗なら非ゼロ終了**する。`--all` 自体は全体 atomic にしない（project mode は rollback 非公開で意味論が崩れるため・→ ADR-0013）。
...
各フィールドは `lib/types.nix` の entry submodule（`lib.types`）として定義され、`mkManifest` の `evalModules` が検査・デフォルト適用する（→ ADR-0010）。submodule は **strict**（未知キー拒否）で、タイポや旧名（`name` / `source` / `dir`・→ ADR-0008, ADR-0014）は評価時エラーになる。識別子（target）の一意性は attrset のキー重複不可で native に担保される（→ ADR-0014）。
...
| `mode` | `"symlink"` \| `"copy"` | 配置種別（デフォルト適用後）|

> entry の `name` フィールドは廃止した（→ ADR-0014）。symlink farm の GC アンカー名は `target` をサニタイズして用いる。

### symlink farm との対応

derivation は `manifest.json` と symlink farm を含む。farm は **GC アンカー専用**で、engine が配置に使う値は `manifest.json` の解決済み `src` 文字列（→ ADR-0010）。

- store-backed entry（`srcKind = "store"`）は farm に store パスへの symlink アンカーを持ち、profile 世代が GC root として全 store src を掴む。
...
評価時エラー（`root` 省略・不正な型・未知キー・copy+marker・systemRoot 未実装）は `mkManifest` の `evalModules` / `lib.throwIf` が検出する（→ ADR-0010, ADR-0013）。識別子の一意性は entries が attrset のため Nix のキー重複不可で担保され、重複 `name` という評価時エラーは存在しない（→ ADR-0014）。
...
| `mode = "copy"` かつ `src` が out-of-store marker | Nix 評価時にエラー（`lib.throwIf`・意図矛盾・→ ADR-0013）|
