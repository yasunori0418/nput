...
nput --root <path>      # project mode で root を明示上書き（git toplevel を使わない）
...
`nput gitignore` は配置 target を `.gitignore` 向けに列挙して stdout に出力するだけで、ファイルは書き込まない。
`.gitignore` の更新責務はプロジェクト管理者が持つ（→ ADR-0005）。
