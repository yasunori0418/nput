...
渡して起動する**薄いラッパー** derivation を返す（→ ADR-0006）。配置ロジックはラッパーではなくエンジンが所有する。
ファイル内容の生成・変換は行わない。リポジトリの内容をそのまま配置する。standalone では nix profile に乗せた世代管理を行う（→ ADR-0002）。
...
| 引数 | 型 | デフォルト | 説明 |
|---|---|---|---|
| `pkgs` | nixpkgs | — | エンジン（`packages.nput`）の解決と `writeShellApplication` に使用 |
| `name` | string | — | 配置単位名。profile を一意特定する（`mkActivationScript` 1 呼び出し = 1 profile）|
| `entries` | list of entry | — | 配置定義のリスト（後述）|
| `root` | marker \| string | `$HOME`（home mode）| 配置先の基準パス。`projectRoot` マーカーで project mode、絶対パス文字列で固定 root（→ ADR-0004, ADR-0005）|

`root` は配置先の基準を選ぶ公開引数（→ ADR-0004 改訂）。

| `root` の値 | モード | root の解決 |
|---|---|---|
| 省略 | home mode | 実行時の `$HOME`（standalone / HM 共通）|
| `nput.lib.projectRoot` | project mode | 実行時に git toplevel（`--root` で上書き可）（→ ADR-0005）|
| 絶対パス文字列 | 固定 root | 評価時に確定する絶対パス（将来の system 配置 root = `/` の seam）|

home mode と project mode は世代の扱いが異なる（後述「世代管理仕様」）。

**返り値**

`pkgs.writeShellApplication` で生成したラッパー derivation。`result/bin/nput` として実行可能で、内部は
`exec ${packages.nput}/bin/nput --manifest ${manifest} "$@"` の形でサブコマンド / フラグをエンジンに転送する（→ ADR-0006）。

**CLI（サブコマンド体系・→ ADR-0006）**

```bash
nput                    # = apply（サブコマンド省略時のデフォルト）
nput apply              # 新世代を作って全エントリを適用（明示）
nput apply --dryrun     # dry-run。副作用ゼロで place/replace/remove/conflict/no-op を表示
nput rollback           # 前世代へ戻す（home mode 限定）
nput list-generations   # 世代一覧を表示（home mode 限定）
nput gitignore          # 配置 target を .gitignore 向けに stdout 出力（書き込みはしない）
nput <sub> --root <p>   # project mode で root を明示上書き（git toplevel を使わない・global flag）
```

`nix run .#x`（= apply）/ `nix run .#x -- apply --dryrun` / `nix run .#x -- rollback` のように使う。
...
- `apply --dryrun` は FS 書込・`--set`・flock いずれも行わない読み取り専用。`conflict` があれば非ゼロ終了（CI の事前 gate に使える）（→ ADR-0006）。
- `gitignore` は配置 target を `.gitignore` 向けに列挙して stdout に出力するだけで、ファイルは書き込まない。更新責務はプロジェクト管理者（→ ADR-0005）。
...
配置データ（`manifest.json` + symlink farm を含む derivation）を生成する**純粋関数**（→ ADR-0006）。
`mkActivationScript` が内部で使うが、Nix 評価テスト（nix-unit / namaka）の対象として単体でも公開する。
配置ロジックは持たず、データだけを作る。

```
mkManifest :: { name, entries, root ? <$HOME> } -> derivation
```

- 生成物は profile が指す store オブジェクト。`manifest.json`（`schemaVersion` 付き・エンジンの入力契約）と、
  配置元への明示 symlink farm（GC アンカー）を含む（→ ADR-0006）。
- `manifest.json` は各 entry の解決済み配置元（store パス / out-of-store 絶対パス）・`source`・`target`・`mode`・配置モード（home/project）を記録する。
- 副作用は持たない（profile swap・FS 配置は Go エンジンの実行時責務）。
...
Go エンジンが**ネイティブ FS 操作**で行う（`ln` / `rsync` は使わない・→ ADR-0006）。
...
3. <配置元>/<source> を指す symlink を <root>/<target> に作成（os.Symlink）
...
```
source がディレクトリの場合:
  target が不在のとき: <root>/<target> を作成しネイティブ再帰コピー（mode 保存。<src>/<source>/ → <root>/<target>/）
  target が存在するとき: 何もしない（ユーザー管理に委ねる）

source がファイルの場合:
  target が不在のとき: 親ディレクトリを作成しネイティブコピー（<src>/<source> → <root>/<target>）
...
- **固定 Go エンジン**（`packages.nput`）が実行時の副作用として（→ ADR-0006）:
...
配置・cleanup アルゴリズムは home-manager の `linkGeneration`/`cleanup` を参考に Go で再実装する（`home.file` 自体は再利用しない）。`nix` / `git` 以外はサブプロセスを使わない。
...
host（`home-manager --rollback` 等）に一本化する。`nput rollback` は home mode（standalone）限定。host rollback は旧 config を
...
- **profile は解決済み root でキーする**（例: `~/.local/state/nix/profiles/nput/<roothash>/<name>`）。同一 flake を複数箇所に
...
target を絶対パスに変換する基準（root）の解決方法。配置の実体は全層で nput エンジンが実行する（→ ADR-0003）。
root は `mkActivationScript` の `root` 引数で選ぶ（→ ADR-0004 改訂, ADR-0005）。
...
| mode | `src` | 動作 |
|---|---|---|
| `"symlink"` | path / set | nput エンジンが store link をネイティブ symlink |
| `"symlink"` | marker | nput エンジンが out-of-store symlink をネイティブ作成（HM の mkOutOfStoreSymlink には委譲しない）|
| `"copy"` | path / set | nput エンジンが place-once ネイティブコピー |
...
nix profile による世代管理を行い、`--rollback` / `--list-generations` を提供する（home mode）。

### devShell（配線・→ ADR-0005）

`devShells.<name>` の `shellHook` から nput エンジンを起動する配線。HM モジュールと同型で、配置ロジックは持たず
root（project mode の git toplevel）と activation タイミング（シェル入室）を供給するだけ。

```nix
devShells.default = pkgs.mkShell {
  shellHook = ''
    ${nput.lib.mkActivationScript { inherit pkgs; name = "skills"; root = nput.lib.projectRoot; entries = [ ... ]; }}/bin/nput
  '';
};
```
...
| project mode で git リポジトリ外かつ `--root` 未指定（git toplevel 解決失敗）| スクリプト実行時にエラーで停止 |
| project mode で `git` が PATH に無い | スクリプト実行時にエラーで停止 |
...
| コンポーネント | 依存 |
|---|---|
| `lib/`（`mkManifest` / `mkActivationScript` ラッパー）| nixpkgs.lib のみ（純データ生成。`rsync` 不要）|
| `lib/out-of-store.nix`（`mkOutOfStoreSymlink` / `projectRoot` マーカー）| なし（純粋なマーカー構築子）|
| `cmd/nput` + `internal/`（Go エンジン = `packages.nput`）| Go 標準ライブラリ中心。`buildGoModule` でビルド。runtime に `nix`（profile）/ `git`（toplevel）をサブプロセスで要求（→ ADR-0006）|
...
lib 層は home-manager / NixOS / nix-darwin に依存しない。配置ロジックは Go エンジンが単一の源として所有する（→ ADR-0003, ADR-0006）。
