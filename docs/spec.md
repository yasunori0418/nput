...
- **`normalizeManifest { entries, root } -> attrset`**: `evalModules` 検査・デフォルト適用（`subpath` → `"."` / `mode` → `"symlink"`）+ クロスフィールド `lib.throwIf` チェック（重複 `name` / `mode = "copy"` かつ out-of-store marker / `root = systemRoot` の未実装拒否・→ ADR-0013）+ 内部 marker タグ（`_nputMarker`）→ clean enum（`srcKind` / `rootKind`）変換を行う純データ関数。nix-unit / namaka の単体対象。
