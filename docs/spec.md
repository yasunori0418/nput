...
- **`-v` / `--verbose` を付けたときだけ配置レポートを stderr に出す**（既定沈黙の opt-in・skip 通知・`apply --all` 完了サマリも含む・→ ADR-0031）。**内部実行する nix コマンドの開示は `--debug`** に分離する（冗長度＝`-v` と、デバッグ＝`--debug` を直交させる）。`--json` は MVP では持たない（将来送り）。`--quiet` は既定沈黙化に伴い廃止予定（現在 no-op）。
