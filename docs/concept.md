...
`home.file` 相当（root = `$HOME`）はその一適用に過ぎない（→ ADR-0004）。
...
各エントリは `name` で識別され、配置単位（`mkActivationScript` 1 呼び出し）ごとに独立した
profile・世代系列として管理される。役割ごとに別スクリプトに分けることで、1つの更新が他に波及しない。
...
対象スクリプトだけを再適用することで、他のツールへの影響を完全に排除できる。
...
- 純粋関数が link farm derivation（ストア内の symlink ツリー）を生成し、
  activation スクリプトが `nix-env --profile` で nix profile に登録する。
- 世代番号・GC root・ロールバックを Nix 標準機構から得る。profile symlink の差し替えだけで atomic に切り替わる。
- 粒度は `mkActivationScript` 単位 = 1 profile。役割ごとに独立したロールバック系列を持つ。
...
```
純粋関数:  link farm derivation と activation スクリプトを生成（副作用なし）
...
このため、コアの中心抽象は root を `$HOME` に固定せず一般化する。`home.file`（root = `$HOME`）も
将来の system 配置（root = `/`）も、同じ関数の引数差で到達できるように設計する。

**スコープの線引き（honest な但し書き）**

- 今回の実装スコープは standalone + home-manager をコアとし、root = `$HOME` 固定。root = `/` 配置は将来拡張。
...
配置の実体は全層で **nput 自身のエンジン（`ln` / `rsync`）**が実行する（→ ADR-0003）。
