...
nix のセットアップは全ジョブ共通で composite action `.github/actions/setup-nix`
（`cachix/install-nix-action` + `cachix/cachix-action`・各 action は SHA pin）に閉じ、
`nix develop '.?dir=dev#ci' -c <cmd>` の CI 用 devShell からコマンドを叩く。`e2e` の
ハーネス（`tests/e2e/run.sh`）もこの経路で走る。
