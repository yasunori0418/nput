...
本パイプラインが参照する外部 action は、composite action の中も job の直書きも**コミット SHA で
...
引き受ける「不変な識別子での固定」の実体になる（→ QA-d028e302）。同じ方式を他の workflow
（`test.yml` の `sara` ジョブ・`cachix-push.yml` の投入 workflow・リリース系 2 workflow）も
採るが、それらの担当はそれぞれ INF-659b139d / INF-af33c5a1 / INF-9878e9f5 にある。
