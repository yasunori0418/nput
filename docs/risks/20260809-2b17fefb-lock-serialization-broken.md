...
level: high
---
# RISK-2b17fefb: flock による直列化が破れ、同一 config への同時実行が互いの配置を壊す

## リスク

同一 `profileDir` に対する apply / rollback / reset は flock で直列化される
（→ REQ-60c6b7ea (2)(a)）。この直列化が破れると、2 つの run が同じ target 集合へ同時に
...
のときだけ try-lock（`LOCK_NB`）でスキップする（→ REQ-1c1526b1）。blocking のはずの経路が
...
TC-4b4709c9（engine 公開 API から見たロック取得モードと保持区間の観測）と
TC-5454b99c（`internal/lock` プリミティブ単体の try / blocking / 再取得）が緩和する。
