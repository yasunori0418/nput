...
- impact: high — 評価時のゲートが形骸化する類型なので、規約の継承則に従い「そのゲートが
  捕まえるはずの regression の impact」を継ぐ。継承元は他の risk item ではなく本 item が直接
  持つ regression、すなわちパス安全性の破れである（`..` や絶対パスが root の外を指したまま
  engine へ届く経路。ゲートを素通りした入力が実行時に何をするかは本 item の「顕在化したときに
  起きること」節が述べる）。書き込み先は nput の記録に残らず原状回復の対象にすらならない。
  ゲート自体が評価時に閉じることを理由に low へは落とさない
