...
非公開の強制そのものは TC-f304d682 が持ち、名指しの拒否（入口の評価で root の種別を
解決した後にしか効かないため実際に評価を通す e2e で見る）と全件列挙からの除外の
両経路を覆う。覆えない範囲（fixed root の拒否・名指しの拒否が engine へ到達しないこと）は
同 TC が残件として挙げている。
...
`--json` 観測）が緩和する。世代の公開／非公開の強制は TC-f304d682（project mode の
世代は公開されず、拒否のときは理由が利用者へ届く）が緩和し、名指しした project mode の
config に対する拒否を CASE-1fc8b8b9 が、全 config を対象とする列挙からの除外を
CASE-2e7b9ea2 が覆う。
