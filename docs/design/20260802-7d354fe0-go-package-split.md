...
- **`internal/` は Go の言語仕様で外部リポジトリから import できない**ため、engine を
  ...
  公開しない意思が構造で表明される
- **テスト対象が `internal/` に集約される**。REQ-6c4e174a が engine に許す外部コマンドは
  nix と git だけで、それ以外は全てネイティブ FS 操作になる。この検証は engine 側の
  ユニットテスト（DSG-836aa5cb）で行うため、検証対象が 1 パッケージに収まっていることが
  テスト戦略の前提になる
...
`docs/design.md`「プロジェクト構成」の `cmd/nput/` / `internal/` の項（L50-52）と、
