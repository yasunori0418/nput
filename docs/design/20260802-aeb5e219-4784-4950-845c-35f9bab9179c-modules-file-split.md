...
---
# DSG-aeb5e219-4784-4950-845c-35f9bab9179c: modules は common.nix に共通オプションを集約し、統合層ごとに 1 ファイルへ分ける
...
REQ-fc1c7ce6-dc9d-4dd3-98f5-7877d9f99d10 は「全モジュールが共通オプションの同一集合を公開し、モジュールごとに集合が
...
`common.nix` が持つのは REQ-fc1c7ce6-dc9d-4dd3-98f5-7877d9f99d10 が定める `enable` / `configs`（`entries` を含む）/
`backup` と、素の `entries` の deprecated 糖衣まで。root オプションは REQ-fc1c7ce6-dc9d-4dd3-98f5-7877d9f99d10 の通り
どのモジュールも公開せず、各モジュールが自分の性質で pin する（home-manager → `homeRoot`）。

**`user` オプションだけは `common.nix` に含めず、`nixos.nix` / `nix-darwin.nix` の各
モジュール内で追加定義する**。REQ-c2654ca5-62c2-4e4b-ad67-ffc5468f429b が `user` を要求するのは NixOS / nix-darwin に
限られ、home-manager と standalone は `$HOME` を直接参照するため不要だからである。
共通側へ置くと、使わない層にも空のオプションが生えて「共通集合の同一性」が
かえって曖昧になる。

統合層を 1 層 1 ファイルにするのは、各ファイルが持つのが REQ-c1b3ca5f-d2f7-443c-bc4b-b18413ca97b9 の言う
「エンジンをキックするだけの配線」であって、層ごとに異なるのは activation の
フック名と root の供給元だけだからである（HM の kick 契約は REQ-8085f194-c903-4ecb-abd8-c719fe7b3292、
配線の内容は DSG-98d7fa5d-c590-441a-b682-0ab7afa14233）。`flake-parts.nix` は他と性質が違い、
REQ-c50df875-2cb0-4e72-8a21-858359a11cae が求める「flake-parts 経路が直書きと同一の derivation を生む」ための
transpose を担う。
...
本 item は ADR-0035 が導入した `nput.configs.<name>` を反映した REQ-fc1c7ce6-dc9d-4dd3-98f5-7877d9f99d10 /
REQ-c6891aeb-13c0-4ae7-9ad1-5c343735266a の現行規範に合わせている。
