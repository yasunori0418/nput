...
- **分類層を fake FS の table-driven で覆える**。TP-e7c25263 が「FS に一切触れずに判断する層は
  ユニットレベルで table-driven に覆う」と定める対象を、この分割が作る。判定の入力
  （記録あり / なし × 実体が symlink / 通常ファイル / ディレクトリ / 不在 × リンク先が記録どおり /
  別物）は FS と違って列挙可能で、table のほうが安上がりで網羅も効く
