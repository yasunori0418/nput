...
「あり（リンク先のみ）」）。この限定の規範は `docs/spec.md`「配置動作仕様」節にあり、
当該節を分割する後続 PR（#209-PR4）で item 化する。

配置動作そのもの（symlink / copy の FS 操作・place-once の意味）と世代管理の機構は
`docs/spec.md`「配置動作仕様」「世代管理仕様」の要求であり、当該節を分割する後続 PR
（#209-PR4）で item 化する。本 item は method の値と、それが選ぶ配置方法・世代管理の
有無までを規定する。
