...
specification_ja: |
  ...
  強いられないようにするため）。列挙する集合は `--dryrun` が報告する集合と同一とする。
  各行には conflict 種別に応じた 1 行の対処ガイダンスを付けなければならない。停止時に
  返すエラーは件数を持つ集約メッセージ 1 本とし、個々の conflict の詳細はそのメッセージ
  本文ではなく直前の列挙が担う。この報告によって終了コードを変えてはならず、機械可読
