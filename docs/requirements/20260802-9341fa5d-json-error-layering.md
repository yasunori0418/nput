...
specification: |
  ...
  failures before any subject has been fixed. Both SHALL always coexist with the
...
specification_ja: |
  ...
  （subject が 1 つも確定していない段階の失敗）のみとする。どちらも stderr の人間向け
...
> 履歴の注記。`--all` 実行時のエラー層の振り分けは REQ-059eb4d5、item 起因エラーを
> `results[i].errors[]` へ重複させないことは REQ-2ea19863 の担当。
