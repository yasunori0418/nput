...
specification: |
  ...
  SHALL remove it before placement (PreRemove, silently) and then newly place the copy.
...
specification_ja: |
  ...
  記録通りであるとき、engine は配置前に除去（PreRemove・silent）してから copy を新規配置
  しなければならない。on-disk が記録と不一致（readlink drift）のときは移行せず、通常の
