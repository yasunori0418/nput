...
- `- 関連:` のうち **ADR 参照**は `references`、**GitHub Issue 参照**は `issues`、**`docs/*.md` 参照**は
  ...
  - **他リポジトリの Issue は逆に `issues` へ入れる**。`owner/repo#N` 形式にすれば ID 空間の衝突が無く
    曖昧さが残らないため（例: ADR-0043 の `yasunori0418/niface#1`）。ADR とは扱いが逆になる。
  - `起点:` 行の Issue は原則 `origin` のテキストに残すが、**その ADR の直接の起点となった Issue は
    `issues` にも入れる**（例: ADR-0026 の `#17`）。経緯として言及されるだけの Issue は入れない
    （例: ADR-0044 / ADR-0045 の「旧 epic #167 から統合された」の `#167`）。
...
- `references` は**後発 → 先行の片方向**に張る。同一ペアに `references` を相互に張ると `sara check` が cycle と
  ...
  - この結果、**先行 ADR が後から追記した前方参照（本文 `関連:` にある、起票順で後発の ADR）は `references`
    ...
    **後発側の本文 `関連:` にその辺が無ければどちらにも現れない**（ADR-0038 → 0040 が該当。ADR-0040 の
    ...
    ではない。ADR を増やしたときはこのリストも更新する。現在の該当は 9 本 16 辺:
