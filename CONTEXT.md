...
**subpath**:
**entry** で **src** 内のどのパスを取り出すかを表す相対パス。file / dir 両対応。デフォルト `"."`。リポジトリ全体は **subpath を省略**するのが canonical（`subpath = "."` は明示形）。評価時に確定する subpath 選択であり、糖衣 marker（`wholeRepo` 等）は足さない（marker は実行時解決の種別を運ぶ入れ物・→ ADR-0007, ADR-0008）。`listFilesInRepo` の引数も同名（旧 `dir` を統一）。
